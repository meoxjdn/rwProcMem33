...
// [MVP 核心修改 1/4]: 新增专用于接收目标地址的 IOCTL 命令字
#define IOCTL_HWBP_SET_MVP_ADDR                     _IOR(MAJOR_NUM, 21, char*)

static atomic64_t g_hook_pc;

// [MVP 核心修改 2/4]: 定义原子变量，安全地在内核中断上下文中保存目标地址
static atomic64_t g_mvp_target_addr;
...
static void hwbp_handler(struct perf_event *bp,
	...
	citerator iter;
	uint64_t hook_pc;
	uint64_t mvp_addr; // [MVP 声明变量]

	// 建议在实战中注释掉这行 printk，否则高频触发会占满 dmesg 导致卡顿
	// printk_debug(KERN_INFO "hw_breakpoint HIT!!!!! bp:%px, pc:%px, id:%d\n", bp, regs->pc, bp->id);

	// [MVP 核心修改 3/4]: 极速拦截与寄存器篡改
	mvp_addr = atomic64_read(&g_mvp_target_addr);
	if (mvp_addr != 0 && regs->pc == mvp_addr) {
		// 目标命中：直接篡改寄存器，模拟你的 OFF_MAXHP 逻辑
		regs->regs[0] = 1;         // X0 = 1
		regs->pc = regs->regs[30]; // PC = LR (直接函数返回)
		
		// 关键点：直接 return，截断原生回调，不进入单步流程，不上报 SIGTRAP
		return; 
	}
...
// ... 省略原有的 OnIoctlOpenProcess 等常规函数 ...
static long OnIoctlOpenProcess(unsigned long arg) {
	int64_t pid = 0;
	struct pid * proc_pid_struct = NULL;
	if (copy_from_user(&pid, (void __user *)arg, 8)) { return -EINVAL; }
	proc_pid_struct = get_proc_pid_struct(pid);
	if (!proc_pid_struct) { return -EINVAL; }
	if (copy_to_user((void*)arg, &proc_pid_struct, 8)) { return -EINVAL; }
	return 0;
}

static long OnIoctlCloseProcess(unsigned long arg) {
	struct pid * proc_pid_struct = NULL;
	if (copy_from_user((void*)&proc_pid_struct, (void*)arg, 8)) { return -EFAULT; }
	release_proc_pid_struct(proc_pid_struct);
	return 0;
}

static long OnIoctlGetCpuNumBrps(void) { return getCpuNumBrps(); }
static long OnIoctlGetCpuNumWrps(void) { return getCpuNumWrps(); }
...
static long OnIoctlInstProcessHwbp(unsigned long arg) {
	...
	if (copy_from_user((void*)&user_data, (void*)arg, 24)) { return -EFAULT; }
	...
	mutex_unlock(&g_hwbp_handle_info_mutex);
	if (copy_to_user((void*)arg, &hwbp_handle_info.sample_hbp, 8)) { return -EINVAL; }
	return 0;
}
...
static long OnIoctlUninstProcessHwbp(unsigned long arg) {
	...
	citerator iter;
	bool found = false;
	if (copy_from_user(&sample_hbp, (void __user *)arg, 8)) { return -EFAULT; }
...
static long OnIoctlGetHwbpHitCount(unsigned long arg) {
	...
	struct buf_layout user_data = {0};
	citerator iter;
	if (copy_from_user(&user_data.sample_hbp, (void __user *)arg, 8)) { return -EFAULT; }
	mutex_lock(&g_hwbp_handle_info_mutex);
	for (iter = cvector_begin(g_hwbp_handle_info_arr); iter != cvector_end(g_hwbp_handle_info_arr); iter = cvector_next(g_hwbp_handle_info_arr, iter)) {
		struct HWBP_HANDLE_INFO * hwbp_handle_info = (struct HWBP_HANDLE_INFO *)iter;
	...
	mutex_unlock(&g_hwbp_handle_info_mutex);
	if (copy_to_user((void*)arg, &user_data, 24)) { return -EINVAL; }
	return 0;
}

static long OnIoctlSetHookPc(unsigned long arg) {
	uint64_t pc = 0;
	if (copy_from_user(&pc, (void __user *)arg, 8)) { return -EFAULT; }
	atomic64_set(&g_hook_pc, pc);
	return 0;
}

// [MVP 核心修改 4/4]: 实现接收目标地址的函数
static long OnIoctlSetMvpAddr(unsigned long arg) {
	uint64_t addr = 0;
...
static ssize_t hwBreakpointProc_read(struct file* filp, char __user* buf, size_t size, loff_t* ppos) {
	...
	if (copy_from_user((void*)&sample_hbp, buf, 8)) { return -EFAULT; }
	copy_pos = (size_t)buf;
	...
	for (iter = cvector_begin(g_hwbp_handle_info_arr); iter != cvector_end(g_hwbp_handle_info_arr); iter = cvector_next(g_hwbp_handle_info_arr, iter)) {
		...
		if (hwbp_handle_info->sample_hbp == sample_hbp && hwbp_handle_info->hit_item_arr) {
			...
			for (child = cvector_begin(hwbp_handle_info->hit_item_arr); child != cvector_end(hwbp_handle_info->hit_item_arr); child = cvector_next(hwbp_handle_info->hit_item_arr, child)) {
				...
				if (copy_to_user((void*)copy_pos, hit_item, sizeof(struct HWBP_HIT_ITEM))) { break; }
...
static inline long DispatchCommand(unsigned int cmd, unsigned long arg) {
	switch (cmd) {
	case IOCTL_HWBP_OPEN_PROCESS: 			return OnIoctlOpenProcess(arg);
	case IOCTL_HWBP_CLOSE_HANDLE: 			return OnIoctlCloseProcess(arg);
	case IOCTL_HWBP_GET_NUM_BRPS: 			return OnIoctlGetCpuNumBrps();
	case IOCTL_HWBP_GET_NUM_WRPS: 			return OnIoctlGetCpuNumWrps();
	case IOCTL_HWBP_INST_PROCESS_HWBP: 		return OnIoctlInstProcessHwbp(arg);
	case IOCTL_HWBP_UNINST_PROCESS_HWBP: 	return OnIoctlUninstProcessHwbp(arg);
	case IOCTL_HWBP_SUSPEND_PROCESS_HWBP: 	return OnIoctlSuspendProcessHwbp(arg);
	case IOCTL_HWBP_RESUME_PROCESS_HWBP: 	return OnIoctlResumeProcessHwbp(arg);
	case IOCTL_HWBP_GET_HWBP_HIT_COUNT: 	return OnIoctlGetHwbpHitCount(arg);
	case IOCTL_HWBP_SET_HOOK_PC:			return OnIoctlSetHookPc(arg);
	// 将我们新写的指令分发下去
	case IOCTL_HWBP_SET_MVP_ADDR:           return OnIoctlSetMvpAddr(arg);
...
static loff_t hwBreakpointProc_llseek(struct file* filp, loff_t offset, int orig) {
	unsigned int cmd = 0;
	if (!!copy_from_user((void*)&cmd, (void*)offset, sizeof(unsigned int))) { return -EINVAL; }
	return DispatchCommand(cmd, offset + sizeof(unsigned int));
}

static ssize_t hwBreakpointProc_write(struct file* filp, const char __user* buf, size_t size, loff_t *ppos) { return 0; }
...
int __init hwBreakpointProc_dev_init(void) {
	...
	g_hwbp_handle_info_arr = cvector_create(sizeof(struct HWBP_HANDLE_INFO));
	if(!g_hwbp_handle_info_arr) { return -ENOMEM; }
	...
	g_hwBreakpointProc_devp->pcdev = __kmalloc(sizeof(struct cdev) * 3, GFP_KERNEL);
