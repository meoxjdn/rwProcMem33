...
static void hwbp_handler(struct perf_event *bp,
	...
	if (mvp_addr != 0 && regs->pc == mvp_addr) {
		printk_debug(KERN_INFO "MVP MATCH! Executing Kernel Tampering for address: %px\n", mvp_addr);
		regs->regs[0] = 1;         // 修改 X0 寄存器为 1
		regs->pc = regs->regs[30]; // 将 PC 修改为 LR，实现 Ret 返回
		return;                    // 截断原生流程，防止被检测或抛出异常
	}
...
static inline long DispatchCommand(unsigned int cmd, unsigned long arg) {
	...
	// 【新增代码：分发刚才定义的新指令】
	case IOCTL_HWBP_SET_MVP_ADDR:
		printk_debug(KERN_INFO "IOCTL_HWBP_SET_MVP_ADDR\n");
		return OnIoctlSetMvpAddr(arg);
	default:
		return -EINVAL;
	}
	return -EINVAL;
